...
export default function Spectator({ sessionId }: { sessionId: string }) {
  ...
  const rows = [...(s.players || [])].sort((a, b) => (b.totalPoints || 0) - (a.totalPoints || 0))
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex items-end justify-between">
          <div>
            ...
            <div className="text-neutral-400">Game: {s.game} • Players: {rows.length}</div>
          </div>
        ...
        <div className="rounded-2xl border border-neutral-800">
          {rows.map((p, i) => (
            <div key={p.uid}
              ...
              <div className="w-16 text-right text-xl font-semibold">{p.totalPoints || 0}</div>
            </div>
          ))}
        </div>
      </div>
